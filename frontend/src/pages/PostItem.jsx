...
export default function PostItem() {
    ...
    const handleUpload = async (e) => {
        ...
        if (files.length + images.length > 5) return error('最多只能上传5张图片')
                    ...
                    {({ values, setFieldValue, errors, touched }) => (
                        <Form className="space-y-4">
                            ...
                            <div className="border-2 border-dashed border-gray-300 rounded p-4 text-center bg-gray-50">
                                ...
                                />
                                <p className="text-sm text-gray-500 mt-2">{uploading ? '上传中...' : '点击上传 JPG/PNG 图片'}</p>
                            </div>
                                ...
                                <div className="grid grid-cols-3 gap-3 mt-3">
                                    {images.map(url => (
                                        <div key={url} className="relative group">
                                            <img src={`http://localhost:5000${url}`} alt="上传图" className="h-24 w-full object-cover rounded shadow" />
                                            <button
                                                type="button"
